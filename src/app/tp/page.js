...
export default function ThreeP() {
  ...
  const validateTask = (task) => {
    ...
    task.subtasks.forEach((subtask, index) => {
      ...
      if (isNaN(o) || o <= 0) {
        taskErrors.push(`Subtask ${index + 1}: Optimistic estimate must be a positive number`);
      }
      if (isNaN(m) || m <= 0) {
        taskErrors.push(`Subtask ${index + 1}: Most likely estimate must be a positive number`);
      }
      if (isNaN(p) || p <= 0) {
        taskErrors.push(`Subtask ${index + 1}: Pessimistic estimate must be a positive number`);
      }
      if (!isNaN(o) && !isNaN(p) && o > p) {
         taskErrors.push(`Subtask ${index + 1}: Pessimistic must be greater than or equal to optimistic`);
      }
       // Optional: Check most likely is between optimistic and pessimistic
       if (!isNaN(o) && !isNaN(m) && !isNaN(p) && (m < o || m > p)) {
         taskErrors.push(`Subtask ${index + 1}: Most likely estimate should typically be between optimistic and pessimistic`);
       }
    });
        ...
        <main className="pixel-main">
          ...
          <section className="pixel-header">
            ...
            <p className="pixel-subtitle">
              Estimate your project&apos;s duration by entering optimistic, pessimistic, and most likely values. {/* Fixed unescaped quote */}
              ...
              {Array.isArray(tasks) && tasks.map((task, taskIndex) => (
                <div key={taskIndex} className="pixel-card">
                  <div className="pixel-task-header">
                    <div className="pixel-input-group">
                      <input
                        ...
                        className={`pixel-text-input ${formSubmitted && !task.name.trim() ? 'pixel-input-error' : ''}`}
                      ...
                      />
                       ...
                       {formSubmitted && !task.name.trim() && (
                           <p className="pixel-error-message" role="alert">Task name is required</p>
                       )}
                    </div>
                    <button
                  ...
                  <div className="pixel-subtask-container">
                    ...
                    {Array.isArray(task.subtasks) && task.subtasks.map((subtask, subtaskIndex) => (
                      <div key={subtaskIndex} className="pixel-subtask">
                        <div className="pixel-input-group">
                          <input
                            type="number"
                            // Apply error class based on validation rules and form submission
                            className={`pixel-number-input ${(formSubmitted && (isNaN(parseFloat(subtask.optimistic)) || parseFloat(subtask.optimistic) <= 0)) ? 'pixel-input-error' : ''}`}
                            ...
                            aria-invalid={formSubmitted && (isNaN(parseFloat(subtask.optimistic)) || parseFloat(subtask.optimistic) <= 0)}
                            min="0"
                            step="0.1"
                          />
                           {/* Display inline error */}
                           {formSubmitted && (isNaN(parseFloat(subtask.optimistic)) || parseFloat(subtask.optimistic) <= 0) && (
                                <p className="pixel-error-message" role="alert">Must be positive number</p>
                           )}
                        </div>
                        ...
                        <div className="pixel-input-group">
                          <input
                            type="number"
                            className={`pixel-number-input ${(formSubmitted && (isNaN(parseFloat(subtask.mostLikely)) || parseFloat(subtask.mostLikely) <= 0)) ? 'pixel-input-error' : ''}`}
                            ...
                            aria-label="Most likely estimate"
                             aria-invalid={formSubmitted && (isNaN(parseFloat(subtask.mostLikely)) || parseFloat(subtask.mostLikely) <= 0)}
                            min="0"
                            step="0.1"
                          />
                           {/* Display inline error */}
                           {formSubmitted && (isNaN(parseFloat(subtask.mostLikely)) || parseFloat(subtask.mostLikely) <= 0) && (
                                <p className="pixel-error-message" role="alert">Must be positive number</p>
                           )}
                        </div>
                        ...
                        <div className="pixel-input-group">
                          <input
                            type="number"
                             className={`pixel-number-input ${(formSubmitted && (isNaN(parseFloat(subtask.pessimistic)) || parseFloat(subtask.pessimistic) <= 0 || (parseFloat(subtask.optimistic) > 0 && parseFloat(subtask.pessimistic) > 0 && parseFloat(subtask.optimistic) > parseFloat(subtask.pessimistic)))) ? 'pixel-input-error' : ''}`}
                            ...
                            aria-label="Pessimistic estimate"
                             aria-invalid={formSubmitted && (isNaN(parseFloat(subtask.pessimistic)) || parseFloat(subtask.pessimistic) <= 0 || (parseFloat(subtask.optimistic) > 0 && parseFloat(subtask.pessimistic) > 0 && parseFloat(subtask.optimistic) > parseFloat(subtask.pessimistic)))}
                            min="0"
                            step="0.1"
                          />
                           {/* Display inline error */}
                           {formSubmitted && (isNaN(parseFloat(subtask.pessimistic)) || parseFloat(subtask.pessimistic) <= 0) && (
                                <p className="pixel-error-message" role="alert">Must be positive number</p>
                           )}
                           {formSubmitted && (parseFloat(subtask.optimistic) > 0 && parseFloat(subtask.pessimistic) > 0 && parseFloat(subtask.optimistic) > parseFloat(subtask.pessimistic)) && (
                                <p className="pixel-error-message" role="alert">Pessimistic must be {'>'}= Optimistic</p>
                           )}
                        </div>
              ...
              <>
                <div className="pixel-results-card">
                  <div className="pixel-result-item">
                    <span className="pixel-result-label">Total Tasks with Estimates:</span> {/* Clarified label */}
                  ...
                  <div className="pixel-result-item">
                    ...
                    <span className="pixel-result-value">{typeof totalEstimation === 'number' ? totalEstimation.toFixed(2) : '0.00'} hrs</span>
                  </div>
                </div>
                ...
                <div className="pixel-button-group">
                  <button
                    ...
                    disabled={!Array.isArray(tasks) || tasks.length === 0 || tasks.every(task => !Array.isArray(task.subtasks) || task.subtasks.length === 0)}
                  >
